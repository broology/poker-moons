...
export default async function (
    ...
    cli(
        `sudo docker buildx build . -t "${context.projectName}:${options.tag}" --target="${context.projectName}" --memory=500m -f ./apps/${context.projectName}/Dockerfile --platform linux/arm64`,
    );

    // Before we can push to ecr, we need to authenticate with it
    cli(
        `sudo docker login -u AWS -p $(aws ecr get-login-password --region ${options.region}) ${options.ecrRegistryUrl}`,
    );

    // Tag Image
    cli(`sudo docker tag "${context.projectName}:${options.tag}" "${appRegistry}:${options.tag}"`);

    // Push docker image
    cli(`sudo docker push "${appRegistry}:${options.tag}"`);
