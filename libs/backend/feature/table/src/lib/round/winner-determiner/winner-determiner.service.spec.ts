...
describe('WinnerDeterminerService', () => {
    ...
    const round = mockRound({ pot: 1000, cards: [mockCard({ suit: 'clubs', rank: '10' }), mockCard({ suit: 'diamonds', rank: '10' }), mockCard({ suit: 'hearts', rank: '10' }), mockCard({ suit: 'spades', rank: '10' }), mockCard({ suit: 'clubs', rank: '5' })] });
    const player1 = mockPlayer({ id: 'player_1', username: 'Levi', cards: [mockCard({ suit: 'clubs', rank: '2' }), mockCard({ suit: 'hearts', rank: '3' })] });
    const player2 = mockPlayer({ id: 'player_2', username: 'Bob', cards: [mockCard({ suit: 'spades', rank: '2' }), mockCard({ suit: 'hearts', rank: '5' })] });
    ...
    describe('determineWinner', () => {
        ...
        it('should throw bad request exception if any player does not have 2 cards', async () => {
            const invalidPlayer = mockPlayer({ id: 'player_1', username: 'Levi', cards: [mockCard({ suit: 'clubs', rank: '2' })] });
